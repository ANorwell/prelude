...
;; %s
;; ^-^----------^-^--------------^-^-------^-^-----------^-^--------^-^--------^-^--------^-^-----------^-^-----
;; _q_ quit     _o_ testHydra    _p_ parse _u_ testOnly  _a_ repeat _n_ no-op  _i_ edit   _s_ sbt-shell _f_ it:test _e_ eof
;; _c_ compile  _y_ test:compile _t_ test  _k_ testQuick _r_ run    _l_ clean  _d_ reload _v_ history   _m_ it:compile _h_ help
;; " switcher)
;;         ,@heads)))
...
;; (defun sbt-hydra-command:it-test-compile (project)
;;   `((sbt-hydra:run-project-command "it:test::compile" ,project) nil))
...
;; m - it:test::compile - execute 'compileAll' command for active project
;; f - it:test - execute 'testAll' command for active project
...
;;                         (list (sbt-hydra-command:it-test-compile current-project))
...
;;          (keys '("l" "c" "t" "k" "r" "y" "e" "q" "n" "p" "o" "s" "d" "a" "u" "m" "f" "i" "h" "v"))
